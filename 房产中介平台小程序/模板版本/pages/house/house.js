...
Page({
  ...
  data: {
    ...
    czfs: [
      { label: '不限', opt: [], active: true },
      { label: '整租', opt: [{ opertionType: 'equal', opertionValue: '整租' }], active: false },
      { label: '合租', opt: [{ opertionType: 'equal', opertionValue: '合租' }], active: false },
    ],
    query: {
      fields: [],
      wheres: [
  ...
  reset() {
    ...
    if (this.data.screenFlag == 2) {
      ...
      var temp = []
      for (var i in this.data.query.wheres) {
        if (this.data.query.wheres[i].value == 'config.area' || this.data.query.wheres[i].value == "config.houseType" || this.data.query.wheres[i].value == "config.orientation" || this.data.query.wheres[i].value == "config.decorate" || this.data.query.wheres[i].value == "config.elevator") {
          temp.push(i)
        }
      }
